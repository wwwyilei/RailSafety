...
def create_overlay_visualization(image, combined_detections, depth_map, class_labels):
    """在原图和深度图上叠加检测和分割结果"""
    import cv2
    import numpy as np
    import matplotlib as mpl
    ...
    depth_vis = cv2.cvtColor(cv2.resize(depth_colormap, (image.shape[1], image.shape[0])), cv2.COLOR_RGB2BGR)

    # 在两个图上添加检测结果
    labels = [f"{class_labels[class_id]}" for class_id in combined_detections.class_id]
    original_vis = mask_annotator.annotate(scene=original_vis, detections=combined_detections)
    original_vis = bounding_box_annotator.annotate(scene=original_vis, detections=combined_detections)
    
    depth_vis = mask_annotator.annotate(scene=depth_vis, detections=combined_detections)
    depth_vis = bounding_box_annotator.annotate(scene=depth_vis, detections=combined_detections)

    return original_vis, depth_vis

def save_overlay_visualization(image_path, output_dir, combined_detections, depth_map, class_labels):
    """保存叠加后的可视化结果"""
    import os
    import cv2
    ...
    original_vis, depth_vis = create_overlay_visualization(image, combined_detections, depth_map, class_labels)
    
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        
    base_name = os.path.splitext(os.path.basename(image_path))[0]
    cv2.imwrite(os.path.join(output_dir, f"{base_name}_overlay_original.png"), original_vis)
    cv2.imwrite(os.path.join(output_dir, f"{base_name}_overlay_depth.png"), depth_vis)
    ...
    try:
        ...
        for image_path in tqdm(image_paths, desc="Processing Images"):
            try:
                ...
                person_bbox_image = image.copy()
                person_mask_image = image.copy()
                
                for i, (xyxy, class_id, mask) in enumerate(zip(combined_detections.xyxy, combined_detections.class_id, combined_detections.mask)):
                    if class_labels[class_id] == 'person':
                        # Bbox only
                        person_bbox_image = bounding_box_annotator.annotate(
                            scene=person_bbox_image,
                            detections=sv.Detections(
                                xyxy=np.array([xyxy]),
                                confidence=np.array([combined_detections.confidence[i]]),
                                class_id=np.array([class_id])
                            )
                        )
                        # Mask only
                        person_mask_image = mask_annotator.annotate(
                            scene=person_mask_image,
                            detections=sv.Detections(
                                mask=np.array([mask]),
                                xyxy=np.array([xyxy]),
                                confidence=np.array([combined_detections.confidence[i]]),
                        ...
                        # Combined
                        person_image = mask_annotator.annotate(scene=person_image, 
                            detections=sv.Detections(
                                mask=np.array([mask]),
                                xyxy=np.array([xyxy]),
                                confidence=np.array([combined_detections.confidence[i]]),
                        ...
                        person_image = bounding_box_annotator.annotate(
                            scene=person_image,
                            detections=sv.Detections(
                                xyxy=np.array([xyxy]),
                                confidence=np.array([combined_detections.confidence[i]]),
                ...
                cv2.imwrite(os.path.join(output_dir, f"{base_name}_person_bbox.png"), person_bbox_image)
                cv2.imwrite(os.path.join(output_dir, f"{base_name}_person_mask.png"), person_mask_image)
                cv2.imwrite(os.path.join(output_dir, f"{base_name}_person_combined.png"), person_image)

                # Both bbox and segmentation
                combined_image = image.copy()
                combined_image = mask_annotator.annotate(scene=combined_image, detections=combined_detections)
                combined_image = bounding_box_annotator.annotate(scene=combined_image, detections=combined_detections)
                cv2.imwrite(os.path.join(output_dir, f"{base_name}_combined.png"), combined_image)

                # Only rail edges
                rail_edge_image = image.copy()
                if len(combined_detections.mask) > 0:
                    rail_mask = combined_detections.mask[0]
                    edges = find_rail_edges(rail_mask)
                    for edge in edges:
                        cv2.circle(rail_edge_image, (edge['left'], edge['y']), 2, (0, 0, 255), -1)
                        cv2.circle(rail_edge_image, (edge['right'], edge['y']), 2, (0, 0, 255), -1)
                cv2.imwrite(os.path.join(output_dir, f"{base_name}_rail_edges.png"), rail_edge_image)

                # Only rail mask
                rail_mask_image = image.copy()
                if len(combined_detections.mask) > 0:
                    rail_mask_viz = np.zeros_like(image)
                    rail_mask_viz[combined_detections.mask[0]] = [203, 192, 255]  # 使用肉色
                    rail_mask_image = cv2.addWeighted(image, 0.7, rail_mask_viz, 0.3, 0)
                cv2.imwrite(os.path.join(output_dir, f"{base_name}_rail_mask.png"), rail_mask_image)

                # Only depth map
                cv2.imwrite(os.path.join(output_dir, f"{base_name}_depth.png"), depth_colormap_bgr)

                # Depth map with annotations
                cv2.imwrite(os.path.join(output_dir, f"{base_name}_depth_annotated.png"), annotated_depth_image)

                # Final visualization with distance measurements
                cv2.imwrite(os.path.join(output_dir, f"{base_name}_final.png"), annotated_image)

                save_overlay_visualization(image_path, output_dir, combined_detections, depth_map, class_labels)

                # Save raw depth data
                # np.save(os.path.join(output_dir, f"{base_name}_depth.npy"), depth_visual)
