...
def calculate_instance_depths(detections, depth_map, config):
...
def find_matching_depth_on_rail(rail_mask, depth_map, target_depth, person_foot_position, tolerance=0.2):
    """
...
class ParallelImageProcessor:
    def __init__(self, yolo_model, sam_predictor, depth_anything, rail_detector, config):
        ...
        self.classes = config['detection_settings']['classes']
        self.detection_settings = config['detection_settings']
        self.class_specific_settings = self.detection_settings['class_specific_settings']
        self.gauge_width = config['rail_settings']['gauge_width']
        self.vis_settings = config.get('visualization', {})
        

        self.person_settings = self.class_specific_settings['person']
        self.car_settings = self.class_specific_settings['car']
        self.rail_track_idx = self.classes.index('rail-track')
        self.car_idx = self.classes.index('car')
        

        self.vis_colors = self.vis_settings.get('colors', {
            'projected_point': (255, 0, 255),  # purple
            'matching_point': (0, 255, 0),     # green
            'distance_line': (255, 255, 255)   # white
        })
        
        # Parallel processing settings
        self.executor = ThreadPoolExecutor(max_workers=config.get('hardware', {}).get('num_workers', 3))

    def run_detection(self, image):

        # person detection
        self.yolo_model.set_classes(['person'])
        person_results = self.yolo_model.infer(
            image, 
            confidence=self.person_settings['confidence_threshold'],
            iou=self.person_settings['iou_threshold']
        )
        person_detections = sv.Detections.from_inference(person_results)
        
        # car detection
        self.yolo_model.set_classes(['car'])
        car_results = self.yolo_model.infer(
            image,
            confidence=self.car_settings['confidence_threshold'],
            iou=self.car_settings['iou_threshold']
        )
        car_detections = sv.Detections.from_inference(car_results)
        
        return person_detections, car_detections

    def combine_detections(self, detections, all_masks, image_shape):

        person_detections, car_detections = detections
        
        # rail track detection
        rail_track_xyxy = np.array([[0, 0, image_shape[1], image_shape[0]]])
        rail_track_confidence = np.array([1.0])
        rail_track_class_id = np.array([self.rail_track_idx])
        ...
        if len(person_detections) > 0 and len(car_detections) > 0:
            combined_detections.xyxy = np.vstack((rail_track_xyxy, person_detections.xyxy, car_detections.xyxy))
            combined_detections.confidence = np.concatenate((rail_track_confidence, person_detections.confidence, car_detections.confidence))
            car_detections_class_ids = np.full_like(car_detections.class_id, self.car_idx)
            combined_detections.class_id = np.concatenate((rail_track_class_id, person_detections.class_id, car_detections_class_ids))
        elif len(person_detections) > 0:
            combined_detections.xyxy = np.vstack((rail_track_xyxy, person_detections.xyxy))
            combined_detections.confidence = np.concatenate((rail_track_confidence, person_detections.confidence))
            combined_detections.class_id = np.concatenate((rail_track_class_id, person_detections.class_id))
        elif len(car_detections) > 0:
            car_detections_class_ids = np.full_like(car_detections.class_id, self.car_idx)
            combined_detections.xyxy = np.vstack((rail_track_xyxy, car_detections.xyxy))
            combined_detections.confidence = np.concatenate((rail_track_confidence, car_detections.confidence))
            combined_detections.class_id = np.concatenate((rail_track_class_id, car_detections_class_ids))
        else:
            combined_detections.xyxy = rail_track_xyxy
            combined_detections.confidence = rail_track_confidence
            combined_detections.class_id = rail_track_class_id
        
        return combined_detections

    async def process_image_parallel(self, image, calibration_params=None):

        loop = asyncio.get_event_loop()
        
        # 1. 创建并行任务
        detect_future = loop.run_in_executor(self.executor, self.run_detection, image)
        depth_future = loop.run_in_executor(self.executor, self.run_depth_estimation, image, calibration_params)
        
        # 2. 等待检测和深度估计完成
        detections, depth_map = await asyncio.gather(detect_future, depth_future)
        
        # 3. 执行分割
        segment_future = loop.run_in_executor(
            self.executor,
            self.run_segmentation,
            image,
            detections
        )
        all_masks = await segment_future
        
        # 4. 合并检测结果
        combined_detections = self.combine_detections(detections, all_masks, image.shape)
        
        # 5. 计算实例深度
        instance_depths = await loop.run_in_executor(
            self.executor,
            self.calculate_instance_depths,
            combined_detections,
            depth_map
        )
        
        return combined_detections, depth_map, instance_depths

    def calculate_instance_depths(self, detections, depth_map):

        instance_depths = []
        for mask, xyxy in zip(detections.mask, detections.xyxy):
            if self.classes[detections.class_id[len(instance_depths)]] in ['person', 'car']:
                foot_depth = get_foot_depth(mask, depth_map, xyxy)
                instance_depths.append(foot_depth if foot_depth is not None else np.nan)
            else:
                instance_depths.append(None)
    ...
    def run_depth_estimation(self, image, calibration_params):

        depth = self.depth_anything.infer_image(image, input_size=720)
        ...
        if calibration_params is not None:
            depth = process_depth_map(depth, calibration_params)
        
        return depth

    # def run_segmentation(self, image, detections):

    #     return segment(
    #         sam_predictor=self.sam_predictor,
    #         rail_detector=self.rail_detector,
    #         image=cv2.cvtColor(image, cv2.COLOR_BGR2RGB),
    #         detections=detections[0] if isinstance(detections, tuple) else detections
    #     )
    def run_segmentation(self, image, detections):
        """执行实例分割"""
        person_detections, car_detections = detections
        ...
        if len(person_detections) > 0 and len(car_detections) > 0:
            combined_detections.xyxy = np.vstack((person_detections.xyxy, car_detections.xyxy))
            combined_detections.confidence = np.concatenate((person_detections.confidence, car_detections.confidence))
            combined_detections.class_id = np.concatenate((person_detections.class_id, car_detections.class_id))
        elif len(person_detections) > 0:
    ...
    try:
        for image_path in tqdm(image_paths, desc="Processing Images"):
            try:
                ...
                combined_detections, depth_map, instance_depths = await processor.process_image_parallel(
                    image, calibration_params
                )
                ...
                # Preparing depth map visualizations
                vmax = np.percentile(depth_visual, vis_config.get('depth_percentile', 95))
                
                # Reuse the same colormap object
                with_colormap = getattr(processor, '_colormap_cache', None)
                if with_colormap is None:
                    normalizer = mpl.colors.Normalize(vmin=depth_visual.min(), vmax=vmax)
                    processor._colormap_cache = cm.ScalarMappable(
                        norm=normalizer, 
                        cmap=vis_config.get('depth_colormap', 'Spectral_r')
                    )
                    with_colormap = processor._colormap_cache
                
                # 应用掩码和边界框时，添加类别标签
                labels = [f"{class_labels[class_id]}" for class_id in combined_detections.class_id]
                
                # 缓存对象重复使用
                depth_colormap = (with_colormap.to_rgba(depth_visual)[:, :, :3] * 255).astype(np.uint8)
                ...
                annotated_image = box_annotator.annotate(scene=annotated_image, detections=combined_detections)
                annotated_depth_image = mask_annotator.annotate(scene=annotated_depth_image, detections=combined_detections)
                annotated_depth_image = box_annotator.annotate(scene=annotated_depth_image, detections=combined_detections)


                vis_prep_time = time.time() - vis_start
                ...
                #         # debug_vis = visualize_distance_calculation(image, person_mask, rail_mask, depth_map)
                #         # cv2.imwrite(os.path.join(OUTPUT_DIRECTORY, 
                #         #                        f"{os.path.splitext(image_name)[0]}_distance_{i}.png"), debug_vis)
                ...
                for i, (xyxy, class_id, depth) in enumerate(zip(combined_detections.xyxy, 
                    ...
                    if class_labels[class_id] in ['person', 'car']:
                        ...
                        distance, projected_point, matching_point = calculate_horizontal_distance(
                            rail_mask, 
                            combined_detections.mask[i], 
                            depth_map,
                            gauge_width=processor.gauge_width
                        )
                        
                        if all(x is not None for x in [distance, projected_point, matching_point]):
                            horizontal_distance = distance
                            # Draw on two image
                            for img in [annotated_image, annotated_depth_image]:
                                cv2.circle(img, (projected_point[1], projected_point[0]), 4, 
                                         processor.vis_colors['projected_point'], -1)
                                cv2.circle(img, (matching_point[1], matching_point[0]), 4, 
                                         processor.vis_colors['matching_point'], -1)
                                cv2.line(img, (projected_point[1], projected_point[0]), 
                                       (matching_point[1], matching_point[0]), 
                                       processor.vis_colors['distance_line'], 2)
                ...
                cv2.imwrite(f"{output_dir}/{file_name}_annotated.png", annotated_image)
                cv2.imwrite(f"{output_dir}/{file_name}_depth_annotated.png", annotated_depth_image)
                np.save(f"{output_dir}/{file_name}_depth.npy", depth_visual)
                ...
                print(f"Parallel processing: {process_time:.3f}s")
