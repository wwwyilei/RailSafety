...
class EnhancedRailwayCalibrator:
    ...
    def calibrate(self, image: np.ndarray, use_predefined: bool = False) -> Dict:
        ...
        def adjust_depth_range(depth_map):
            depth_map = depth_map - depth_map.min()  # make sure minimum is 0
            max_depth = max(d for _, _, d in self.manual_points)  # get the maxmum real depth value
            return depth_map * (max_depth / depth_map.max())  # scale to the correct range
    ...
    def visualize_results(self, image: np.ndarray, result: Dict) -> np.ndarray:
        ...
        for x, y, depth in self.manual_points:
            ...
            cv2.putText(display_img, f'{depth:.1f}m', (x+5, y-5),
        ...
        if 'rail_paths' in result and result['rail_paths'][0] is not None:
            ...
            for rail in [left_rail, right_rail]:
                ...
                cv2.polylines(display_img, [points], False, (0, 255, 0), 2)
        
        # Combine visualizations
        top_row = np.hstack((display_img, initial_viz))
...
def main():
    ...
    try:
        ...
        visualization = calibrator.visualize_results(image, result)
